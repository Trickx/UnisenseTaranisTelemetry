-- UnisensE-Taranis-Telemetry-Scripting

-- Copyright (C) 2005 Ollicious, bowdown@gmx.net
-- Copyright (C) 2017 DeWe, http://www.kopterforum.at
-- Copyright (C) 2019 TriCX, https://github.com/Trickx
...
-- ---------------------Available Widgets -------------------------------------
-- Voltage:			"battery"		full colunm widget
-- Voltage: 		"batteryh"		1/2 column widget
-- RSSI: 			"rssi"			full colunm widget
-- RSSI: 			"rssih"			11/2 column widget
-- Flightmode: 		"fmBF"			for flightcontroller Betaflight
-- Flightmode: 		"fmINAV"		for flightcontroller INAV
-- GPS Status: 		"gps"
-- Timer: 			"timer"			should be the counter if Throttle not 0
-- Distance: 		"dist"
-- Height: 			"Altheight"		Alt
-- Height: 			"GPSheight"		GPS
-- Speed: 			"speed"
-- Kompass: 		"heading"
-- Homepoint: 		"hp"
-- Radar: 			"radar"			requires the dummy widget in front
-- IFT Display		"IFT"			requies the dummy widget in front
-- Radar/IFT		:"dummy"		requires 2 columns
...
-- -----------------Automatic LiPo Cell Detection --------------------
-- Note: Please keep care with 5S or 7S Lipos!						--
-- Automatic cell detection is not supported for 5S and 7s Lipos	--
-- => change battype manual in the init section						--
-- Highvolt Lipos are not supported in this moment					--
----------------------------------------------------------------------
...
-- local WgtDefinition = {{"battery"}, {"CAmp", "rpm", "timer"}, {"current", "GPSheight", "speed"}, {"rssi"}} -- for Sven
local WgtDefinition = {{"battery"}, {"CAmp", "rpm", "timer"}, {"current", "Altheight", "vspeed"}, {"rssi"}} --für zum Beispiel Heli's
-- Battery Type ------------------------------------
local battype = 0			-- 0:autodetection (not supported for 5S and 7S), 1..7: static cell number
-- Display Settings --------------------------------
local xOffsetSingle = 35	-- Edit to set the width of single Wgt columns
local displayWidth = 212	-- Display's width in pixel
local displayHeight = 64	-- Display's heigt in pixel
------------------------------------------------------------------------------

-- Don't edit value after this line --
local xOffset
local xOffsetMulti
local numSingleCols
local numMultiCols
...
local function IFTWgt(xPos, yPos, w_type)
	...
	if (w_type > 1) then
		...
		for i = delta - 30, 30 + delta, 15 do
			...
			if
				not ((X1 < -radAH and X2 < -radAH) or (X1 > radAH and X2 > radAH) or (Y1 < -radAH and Y2 < -radAH) or
					(Y1 > radAH and Y2 > radAH))
			 then
		...
		if getVDef("RSSI") > 55 then
			...
			if compare > 40 then
				if math.floor(comp_1 / 4.37) > battype and (comp_1 < (4.37 * 8)) then
					...
					if battype == 7 then
						battype = 8
					end --dont Support 5s&7s Battery, its Danger to Detect
					if battype == 5 then
						battype = 6
					end
				end
...
local function battWgt(xCoord, yCoord, Fullwdgt)
	...
	if ((myAvailV == nil) or (myAvailV < 0)) then
		myAvailV = 0
	end
	if ((myPercent == nil) or (myPercent < 1)) then
		myPercent = 0
	end
	...
	lcd.drawNumber(xCoord + 12, yCoord + 25, battype, 0 + LEFT + SMLSIZE)
	lcd.drawText(lcd.getLastPos(), yCoord + 25, "S", SMLSIZE)
end
